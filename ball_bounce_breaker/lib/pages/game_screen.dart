...
class _GameScreenState extends State<GameScreen> {
  ...
  static const double _paddleSensitivity = 0.06; // higher = more reactive to tilt
  ...
  double get _paddleLeft => (_screenSize.width - _paddlewidth) * (_paddleX + 1) / 2;
  ...
  void _moveBall() {
    ...
    setState(() {
      ...
      final hitsPaddleX = ballCenterX >= _paddleLeft && ballCenterX <= _paddleLeft + _paddlewidth;
      final hitsPaddleY = ballBottom >= _paddleTop && ballBottom <= _paddleTop + _paddleheight;
...
class BlockPainting extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    const topOffset = 50.0;
    const blockHeight = 18.0;
    final blockWidth = (size.width - (columns + 1) * gap) / columns;

    // draw each block that isn't broken
    for (final block in blocks) {
      if (block.broken) continue;

      final top = topOffset + block.row * (blockHeight + gap);
      final left = gap + block.column * (blockWidth + gap);
